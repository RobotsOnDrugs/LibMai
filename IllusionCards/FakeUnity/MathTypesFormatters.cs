...
internal sealed class Vector2Formatter : IMessagePackFormatter<Vector2>, IMessagePackFormatter
{
	...
	{
		ImmutableArray<float> floats = Helpers.UnpackFloats(ref reader, 2);
		return new(floats[0], floats[1]);
	}
}

internal sealed class Vector3Formatter : IMessagePackFormatter<Vector3>, IMessagePackFormatter
{
	...
	{
		ImmutableArray<float> floats = Helpers.UnpackFloats(ref reader, 3);
		return new(floats[0], floats[1], floats[2]);
	}
}

internal sealed class Vector4Formatter : IMessagePackFormatter<Vector4>, IMessagePackFormatter
{
